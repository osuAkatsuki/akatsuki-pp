...
char* m_alloc(ezpp_t ez, int size) {
	char* res;
...
#undef s
	...
	*/
	...
	if (radius < CIRCLESIZE_BUFF_TRESHOLD) {
		...
		if(ez->mods & MODS_RX) scaling_factor *= 1.0f + al_min((CIRCLESIZE_BUFF_TRESHOLD - radius), 7.125f) / 36.25f;
		else                   scaling_factor *= 1.0f + al_min((CIRCLESIZE_BUFF_TRESHOLD - radius), 5.0f) / 50.0f;
	}
...
void taiko_strain(taiko_object_t* cur, taiko_object_t* prev, int mods) {
	float decay;
	float addition = 1.0f;
	float factor = 1.0f;
...
int ezpp_data_dup(ezpp_t ez, char* data, int data_size) {
 ...
 float ezpp_pp(ezpp_t ez) { return ez->pp; }
 float ezpp_stars(ezpp_t ez) { return ez->stars; }
 int ezpp_mode(ezpp_t ez) { return ez->mode; }
 int ezpp_combo(ezpp_t ez) { return ez->combo; }
 int ezpp_max_combo(ezpp_t ez) { return ez->max_combo; }
 int ezpp_mods(ezpp_t ez) { return ez->mods; }
 int ezpp_score_version(ezpp_t ez) { return ez->score_version; }
 float ezpp_aim_stars(ezpp_t ez) { return ez->aim_stars; }
 float ezpp_speed_stars(ezpp_t ez) { return ez->speed_stars; }
 float ezpp_aim_pp(ezpp_t ez) { return ez->aim_pp; }
 float ezpp_speed_pp(ezpp_t ez) { return ez->speed_pp; }
 float ezpp_acc_pp(ezpp_t ez) { return ez->acc_pp; }
 float ezpp_accuracy_percent(ezpp_t ez) { return ez->accuracy_percent; }
 int ezpp_n300(ezpp_t ez) { return ez->n300; }
 int ezpp_n100(ezpp_t ez) { return ez->n100; }
 int ezpp_n50(ezpp_t ez) { return ez->n50; }
 int ezpp_nmiss(ezpp_t ez) { return ez->nmiss; }
 char* ezpp_title(ezpp_t ez) { return ez->title; }
 char* ezpp_title_unicode(ezpp_t ez) { return ez->title_unicode; }
 char* ezpp_artist(ezpp_t ez) { return ez->artist; }
 char* ezpp_artist_unicode(ezpp_t ez) { return ez->artist_unicode; }
 char* ezpp_creator(ezpp_t ez) { return ez->creator; }
 char* ezpp_version(ezpp_t ez) { return ez->version; }
 int ezpp_ncircles(ezpp_t ez) { return ez->ncircles; }
 int ezpp_nsliders(ezpp_t ez) { return ez->nsliders; }
 int ezpp_nspinners(ezpp_t ez) { return ez->nspinners; }
 int ezpp_nobjects(ezpp_t ez) { return ez->nobjects; }
 float ezpp_ar(ezpp_t ez) { return ez->ar; }
 float ezpp_cs(ezpp_t ez) { return ez->cs; }
 float ezpp_od(ezpp_t ez) { return ez->od; }
 float ezpp_hp(ezpp_t ez) { return ez->hp; }
 float ezpp_odms(ezpp_t ez) { return ez->odms; }
 int ezpp_autocalc(ezpp_t ez) { return ez->flags & AUTOCALC_BIT; }

 float ezpp_time_at(ezpp_t ez, int i) {
